...
if ('navigation' in window) {
  ...
  const STORE_SRV_PREF = 'animowebSrvPref';         // { movie: name, tv: name, anime: name }
  const STORE_ANIME_MODE = 'animowebAnimeMode';     // 'mega' | 'demon'

  // Remember which server the user last picked, per content type. Restored on
  // every new playback context so users don't have to re-select their fav.
  function getServerPref(scope) {
    try {
      const m = JSON.parse(localStorage.getItem(STORE_SRV_PREF) || '{}');
      return m && m[scope] ? String(m[scope]) : null;
    } catch (_) { return null; }
  }
  function setServerPref(scope, name) {
    if (!scope || !name) return;
    try {
      const m = JSON.parse(localStorage.getItem(STORE_SRV_PREF) || '{}');
      m[scope] = name;
      localStorage.setItem(STORE_SRV_PREF, JSON.stringify(m));
    } catch (_) {}
  }
  // Anime server-cluster toggle. 'mega' → MegaPlay + scraper workers (embeds).
  // 'demon' → cinematic movie-style servers (ScreenScape / Aurora / …) via
  // the AniList → TMDB mapping. Persists across sessions.
  function getAnimeMode() {
    try { return localStorage.getItem(STORE_ANIME_MODE) === 'mega' ? 'mega' : 'demon'; }
    catch (_) { return 'demon'; }
  }
  function setAnimeMode(mode) {
    try { localStorage.setItem(STORE_ANIME_MODE, mode === 'mega' ? 'mega' : 'demon'); } catch (_) {}
  }
  ...
  // Only used by buildAnimeVAList() (Cast page anime voice-actor aggregation)
  // — the /anime page itself is powered by AniList via anime.js, not this.
  const ANIME = {
    base:    (type, params={}) => {
  ...
  // ───── AniList / AniZip client ─────────────────────────────────────────────
  // AniList's public GraphQL endpoint rate-limits aggressively (HTTP 429),
  // and used to be hit from five different ad-hoc fetch() calls scattered
  // across this file and player.js — none of which checked response.ok or
  // retried, so a single 429 silently broke whatever feature called it
  // ("AniList won't load"). Every anime-related fetch now goes through one
  // of these two helpers: both retry transient failures with backoff and
  // resolve `null` on total failure instead of throwing, matching tmdb()'s
  // proven behaviour so callers never have to special-case a rejection.
  const ANILIST_API = 'https://graphql.anilist.co';
  const ANIZIP_API  = 'https://api.ani.zip/mappings';
  const _alGqlCache = new Map();

  function _sleep(ms) { return new Promise(res => setTimeout(res, ms)); }
  function _alRetryDelay(attempt, retryAfter) {
    const fromHeader = Number(retryAfter);
    if (Number.isFinite(fromHeader) && fromHeader > 0) return fromHeader * 1000;
    return 900 * Math.pow(2, attempt) + Math.random() * 400;
  }

  // POST a GraphQL query to AniList. Retries 429 / 5xx / network errors up to
  // 3 times with backoff (honouring Retry-After when AniList sends one).
  // Resolves the `data` object, or `null` if every attempt failed.
  async function anilistGql(query, variables = {}) {
    const key = JSON.stringify({ query, variables });
    if (_alGqlCache.has(key)) return _alGqlCache.get(key);
      ...
      for (let attempt = 0; attempt <= 3; attempt++) {
        try {
          ...
          if (!r.ok) {
            ...
            if (retriable && attempt < 3) { await _sleep(_alRetryDelay(attempt, r.headers.get('Retry-After'))); continue; }
            return null;
          }
          ...
          if (attempt < 3) { await _sleep(_alRetryDelay(attempt)); continue; }
          return null;
        }
      }
      return null;
    ...
    _alGqlCache.set(key, p);
    const result = await p;
    if (!result) _alGqlCache.delete(key); // don't cache failures — allow a later retry to succeed
    return result;
  }

  // GET an AniList ID's AniZip mapping (TMDB/IMDB/TVDB id + per-episode map).
  // Same retry/backoff treatment; resolves {} (never null/throws) on failure
  // so callers can keep using `(result.tmdb || ...)` without a null-guard.
  const _aniZipCache = new Map();
  async function aniZip(anilistId) {
    const key = String(anilistId);
    if (_aniZipCache.has(key)) return _aniZipCache.get(key);

    const p = (async () => {
      for (let attempt = 0; attempt <= 3; attempt++) {
        try {
          const r = await fetch(`${ANIZIP_API}?anilist_id=${encodeURIComponent(anilistId)}`, {
    ...
    _aniZipCache.set(key, p);
    const result = await p;
    if (!result || !Object.keys(result).length) _aniZipCache.delete(key);
    return result;
  }
  ...
  // ── ani.zip + TMDB caches (drives the TMDB-shaped episode picker) ────────
  const _aniZipMapCache = new Map();
  async function _resolveAniZipMap(anilistId) {
    if (_aniZipMapCache.has(anilistId)) return _aniZipMapCache.get(anilistId);
    const p = aniZip(anilistId).then(j => {
      const m = (j && j.mappings) || {};
      const rawTmdb = m.themoviedb_id || m.themoviedb || null;
      const tmdbId = rawTmdb ? String(rawTmdb).split(',')[0].trim() : null;
      return {
        tmdb: tmdbId || null,
        imdb: m.imdb_id || null,
        tvdb: m.thetvdb_id || null,
        episodes: (j && j.episodes) || {},
      };
    });
    _aniZipMapCache.set(anilistId, p);
    return p;
  }

  const _tmdbTvCache = new Map();
  async function _fetchTmdbTv(tmdbId) {
    if (_tmdbTvCache.has(tmdbId)) return _tmdbTvCache.get(tmdbId);
    const p = tmdb(`/tv/${tmdbId}`).catch(() => null);
    _tmdbTvCache.set(tmdbId, p);
    return p;
  }
  ...
  // used to keep the season-chain walk below from firing a burst of N
  // simultaneous AniList requests for franchises with many related entries,
  // which is exactly the kind of spike that trips AniList's rate limiter.
  ...
  async function _buildSeasonChain(rootId, rootMedia) {
    ...
    for (let depth = 0; depth < 3 && frontier.length; depth++) {
      const results = await _mapWithConcurrency(frontier, 3, _fetchAniRelations);
      const next = [];
  ...
  async function renderBrowse(type) {
    ...
    // Build continue-watching items filtered to this content type
    const progStore = getProgressStore();
    const continueItems = Object.values(progStore)
      .filter(x => x && x.type === type && x.progress && x.progress.duration > 0 && (x.progress.watched / x.progress.duration) < 0.97)
      .sort((a, b) => (b.last_updated || 0) - (a.last_updated || 0))
      .slice(0, 8);

    const continueHtml = continueItems.length ? `
      <section class="section" id="sec-browse-continue">
        <header class="section__head">
          <div>
            <h2 class="section__title">Pick up where you <em>left off</em></h2>
            <p class="section__sub">Your recent ${type === 'movie' ? 'films' : 'series'} — continue anytime.</p>
          </div>
        </header>
        <div class="rail"><div class="rail__strip">${continueItems.map(continueCard).join('')}</div></div>
      </section>` : '';
  ...
  // Episode pager — large anime/TV seasons (50, 100, 500+ eps) destroyed
  // scroll performance and froze the player when every episode card was
  // rendered into the DOM at once. Paginate at EPISODES_PER_PAGE per page
  // with prev/next + jump-to-page. Shared by both the TV season pager and
  // the anime episode pager below — same chunk size, same pager markup.
  const EPISODES_PER_PAGE = 24;

  // Build a compact page-number range: « < 1 … 4 [5] 6 … 12 > »
  ...
  // Slice `totalCount` items into a page of at most `perPage`, clamped to
  // valid bounds. Returns { page, pages, start, end } — `end` is exclusive.
  function paginate(totalCount, page, perPage) {
    const pages = Math.max(1, Math.ceil(totalCount / perPage));
    const p = Math.max(1, Math.min(pages, page || 1));
    const start = (p - 1) * perPage;
    const end = Math.min(start + perPage, totalCount);
    return { page: p, pages, start, end };
  }

  // Shared pager nav markup. `onGoAttr` lets callers namespace the
  // data-attribute so two independent pagers on the same page (e.g. TV
  // season pager vs. an anime episode pager) never collide when wiring
  // click handlers.
  function pagerHtml(p, pages, start, end, total, dataAttr) {
    if (pages <= 1) return '';
    return `
      <nav class="ep-pager" aria-label="Episode pages">
        <button class="ep-pager__btn" data-${dataAttr}="${p-1}" ${p===1?'disabled':''} aria-label="Previous page"><i class="ri-arrow-left-s-line"></i></button>
        ${pageRange(p, pages).map(x =>
          x === '…' ? `<span class="ep-pager__sep">…</span>`
                    : `<button class="ep-pager__btn ${x===p?'active':''}" data-${dataAttr}="${x}">${x}</button>`
        ).join('')}
        <button class="ep-pager__btn" data-${dataAttr}="${p+1}" ${p===pages?'disabled':''} aria-label="Next page"><i class="ri-arrow-right-s-line"></i></button>
        <span class="ep-pager__range">${start+1}–${end} of ${total}</span>
      </nav>`;
  }

  function renderEpisodePage(eps, seasonNum, showId, page, activeSeasonNum, activeEpisodeNum) {
    const { page: p, pages, start, end } = paginate(eps.length, page, EPISODES_PER_PAGE);
    const slice = eps.slice(start, end);

    const cards = slice.map(ep => {
      const isPlaying = activeSeasonNum != null && activeEpisodeNum != null
        && parseInt(activeSeasonNum) === parseInt(seasonNum)
        && parseInt(activeEpisodeNum) === parseInt(ep.episode_number);
      return `
      <a class="episode${isPlaying ? ' episode--playing' : ''}" href="/watch/tv/${showId}/${seasonNum}/${ep.episode_number}" data-link>
    ...
    const pagerHtml_ = pagerHtml(p, pages, start, end, eps.length, 'go');

    return cards + pagerHtml_;
  }

  function wireEpisodes(data, activeSeasonNum, activeEpisodeNum) {
    const seasons = (data.seasons || []).filter(s => s.season_number > 0);
    if (!seasons.length) return;
    ...
    // Scroll to currently playing episode after rendering
    const _scrollToPlayingEp = () => {
      requestAnimationFrame(() => {
        const playing = document.querySelector('#episodesGrid .episode--playing');
        if (playing) try { playing.scrollIntoView({ behavior: 'smooth', block: 'center', inline: 'nearest' }); } catch(_) {}
      });
    };

    // Find which page the active episode is on so we open it directly
    const _findActivePage = (eps, seasonNum) => {
      if (activeSeasonNum == null || activeEpisodeNum == null) return 1;
      if (parseInt(activeSeasonNum) !== parseInt(seasonNum)) return 1;
      const idx = eps.findIndex(e => parseInt(e.episode_number) === parseInt(activeEpisodeNum));
      return idx < 0 ? 1 : Math.ceil((idx + 1) / EPISODES_PER_PAGE);
    };
    ...
    const renderPage = (page) => {
      ...
      if (!grid) return;
      grid.innerHTML = renderEpisodePage(currentEps, currentSeason, data.id, page, activeSeasonNum, activeEpisodeNum);
      // Wire pager
    ...
    const showSeason = async (n) => {
      ...
      renderPage(pageBySeason[n] || _findActivePage(currentEps, n));
    };
  ...
  async function renderWatch(...args) {
    ...
    // Season-aware prev/next for TV
    let _pnh = '';
    if (type === 'tv') {
      const _ep = parseInt(episode,10), _sea = parseInt(season,10);
      const _vs = (data.seasons||[]).filter(s=>s.season_number>0);
      const _mx = _vs.length ? Math.max(..._vs.map(s=>s.season_number)) : _sea;
      const _ct = (_vs.find(s=>s.season_number===_sea)||{}).episode_count||0;
      const _pm = _vs.find(s=>s.season_number===_sea-1);
      const _p = _ep>1
        ? `<a class="btn-ghost ep-nav__btn" href="/watch/tv/${id}/${season}/${_ep-1}" data-link><i class="ri-arrow-left-s-line"></i> Previous</a>`
        : _sea>1
          ? `<a class="btn-ghost ep-nav__btn ep-nav__btn--season" href="/watch/tv/${id}/${_sea-1}/${(_pm&&_pm.episode_count)||1}" data-link><i class="ri-arrow-left-s-line"></i> Season ${_sea-1}</a>`
          : `<button class="btn-ghost ep-nav__btn" disabled><i class="ri-arrow-left-s-line"></i> Previous</button>`;
      const _n = (_ct===0||_ep<_ct)
        ? `<a class="btn-ghost ep-nav__btn" href="/watch/tv/${id}/${season}/${_ep+1}" data-link>Next <i class="ri-arrow-right-s-line"></i></a>`
        : _sea<_mx
          ? `<a class="btn-ghost ep-nav__btn ep-nav__btn--season" href="/watch/tv/${id}/${_sea+1}/1" data-link>Season ${_sea+1} <i class="ri-arrow-right-s-line"></i></a>`
          : `<button class="btn-ghost ep-nav__btn" disabled>Next <i class="ri-arrow-right-s-line"></i></button>`;
      _pnh = `<div class="ep-nav">${_p}${_n}</div>`;
    }
    ...
    if (type === 'tv') {
      ...
      if (watchPage && data.seasons?.length) {
        ...
        if (epsShell) {
          ...
          // Pass active season+episode so wireEpisodes marks it and scrolls to it
          wireEpisodes(data, season, episode);
        }
      }
    } else {
    ...
    if (!samePlayingMovie) {
      ...
      const _pref = getServerPref(type);
      if (type === 'tv') mini.activePlayer.playEpisode(id, parseInt(season), parseInt(episode), { preferredName: _pref });
      else               mini.activePlayer.playMovie(id, { preferredName: _pref });
      mini.activeCtx = {
  ...
  function _langTag(lang) {
    const l = String(lang || 'sub').toLowerCase();
    return l === 'dub' ? 'Dub' : l === 'hindi' ? 'Hindi' : 'Sub';
  }
  ...
  function appendWorkerSources(worker, links) {
    ...
    const tag = _langTag(worker.lang);
  ...
  function resolveMatchedAnimeWorkers(anilistId, episode, season) {
    ...
    workers.forEach((w) => {
      (async () => {
        ...
        // Worker embeds are folded — they remain selectable from the picker
        // but we do NOT auto-switch off the movie-style primary stream.
        // (Was: auto-promoted the first embed to primary.)
        if (!primaryClaimed) primaryClaimed = true;
  ...
  async function renderAnimeWatch(anilistId, episodeNum) {
    ...
    if (!media) {
      $('#view').innerHTML = `<div class="empty">
        <div class="empty__icon"><i class="ri-wifi-off-line"></i></div>
        <h3 class="serif"><em>Couldn't reach AniList.</em></h3>
        <p>It may be rate-limiting requests right now — this usually clears up within a minute.</p>
        <div style="display:flex;gap:10px;justify-content:center;flex-wrap:wrap">
          <button class="btn-primary" id="animeWatchRetry"><i class="ri-refresh-line"></i> Retry</button>
          <a class="btn-ghost" href="/" data-link><i class="ri-arrow-left-line"></i> Back to home</a>
        </div>
      </div>${footer()}`;
      $('#animeWatchRetry')?.addEventListener('click', () => renderAnimeWatch(anilistId, episodeNum), { once: true });
      return;
    }
    ...
    // Anime prev/next — respects totalEps
    const _ap = ep>1
      ? `<a class="btn-ghost ep-nav__btn" href="/watch/anime/${id}/${ep-1}" data-link><i class="ri-arrow-left-s-line"></i> Previous</a>`
      : `<button class="btn-ghost ep-nav__btn" disabled><i class="ri-arrow-left-s-line"></i> Previous</button>`;
    const _an = (!isMovie && ep<totalEps)
      ? `<a class="btn-ghost ep-nav__btn" href="/watch/anime/${id}/${ep+1}" data-link>Next <i class="ri-arrow-right-s-line"></i></a>`
      : `<button class="btn-ghost ep-nav__btn" disabled>Next <i class="ri-arrow-right-s-line"></i></button>`;
    ...
    if (watchPage) {
      ...
      // Page state for the simple AniList-numbered list (renderEpsFor below).
      // Kept outside the function so switching pages doesn't lose position,
      // same pattern as the TV season pager's pageBySeason.
      let epsListPage = Math.max(1, Math.ceil(ep / EPISODES_PER_PAGE));

      const renderEpsFor = (anilistId, eps, currentEp, label, page) => {
        if (!eps || eps < 1) {
        ...
        const { page: p, pages, start, end } = paginate(eps, page || epsListPage, EPISODES_PER_PAGE);
        epsListPage = p;

        const cards = Array.from({ length: end - start }, (_, i) => {
          const n = start + i + 1;
        ...
        grid.innerHTML = cards + pagerHtml(p, pages, start, end, eps, 'eplistgo');
        $$('.ep-pager__btn[data-eplistgo]', grid).forEach(btn => {
          btn.addEventListener('click', () => {
            const go = parseInt(btn.dataset.eplistgo);
            if (Number.isFinite(go)) {
              renderEpsFor(anilistId, eps, currentEp, label, go);
      ...
      // TMDB-driven episode picker: show season pills + rich episode cards
      // (names, thumbnails, air dates from TMDB). Links always stay on
      // /watch/anime/ so demon/mega servers are never lost.
      (async () => {
        try {
          const aniMap = await _resolveAniZipMap(id);
          const tmdbId = aniMap && aniMap.tmdb;
          if (!tmdbId) return;

          const tv = await tmdb(`/tv/${tmdbId}`);
          const seasons = (tv && Array.isArray(tv.seasons) ? tv.seasons : [])
            .filter(s => s && s.season_number > 0 && (s.episode_count || 0) > 0);
          if (!seasons.length) return;
          ...
          // ── Cross-season AniList resolution ─────────────────────────────
          // Every TMDB season usually corresponds to a SEPARATE AniList
          // entry (sequel/side-story). If we keep the current `id` for all
          // season episode links, clicking "S3 E1" just plays the current
          // season's E1 on the same AniList id — the wrong episode entirely.
          //
          // Walk the AniList relation chain rooted at this id, resolve each
          // entry through AniZip, group by their dominant TMDB season, and
          // pick the BEST AniList entry for every TMDB season the show has.
          // Episode links then target the right (aniId, aniEpisode) pair.

          // current playing episode's TMDB position (used for highlight)
          const curEpMap = aniMap.episodes && aniMap.episodes[String(ep || 1)];

          // Map: tmdbSeasonNumber -> { aniId, episodes:{epNum:{seasonNumber,episodeNumber,...}} }
          const seasonToAni = new Map();
          // Always register the current id against whatever season(s) it serves.
          const registerEntry = (aniId, epMap) => {
            if (!epMap || typeof epMap !== 'object') return;
            const counts = Object.create(null);
            Object.keys(epMap).forEach(k => {
              const s = epMap[k] && epMap[k].seasonNumber;
              if (s == null) return;
              counts[s] = (counts[s] || 0) + 1;
            });
            Object.keys(counts).forEach(sStr => {
              const s = parseInt(sStr, 10);
              const prev = seasonToAni.get(s);
              // Prefer the entry that covers the MOST episodes of that season.
              if (!prev || counts[s] > prev._count) {
                seasonToAni.set(s, { aniId, episodes: epMap, _count: counts[s] });
              }
            });
          };
          registerEntry(id, aniMap.episodes);

          // Walk the relation chain in the background — pills render
          // immediately, then upgrade once cross-season links are known.
          const chainPromise = _buildSeasonChain(id, media).then(async (chain) => {
            const others = chain.filter(c => c.id !== id);
            await Promise.all(others.map(async (c) => {
              try {
                const m = await _resolveAniZipMap(c.id);
                if (!m) return;
                // Only consider entries that point at the SAME TMDB show.
                if (!m.tmdb || String(m.tmdb) !== String(tmdbId)) return;
                registerEntry(c.id, m.episodes);
              } catch (_) {}
            }));
          }).catch(() => {});

          // Which TMDB season is the currently-playing episode in?
          let activeIdx = 0;
          if (curEpMap && curEpMap.seasonNumber != null) {
            const i = seasons.findIndex(s => s.season_number === curEpMap.seasonNumber);
            if (i >= 0) activeIdx = i;
          }

          const seasonEpsCache = {};
          const pageBySeasonAnime = {};

          // Resolve the link target for a TMDB (season,episode) pair.
          //   • Prefer the AniList entry registered for that season.
          //   • Within that entry, find the AniList epNum whose mapping
          //     points at the requested (season,episode); fallback to the
          //     TMDB episode number (works when the entry's episodes are
          //     numbered 1..N inside a single season — the common case).
          //   • Final fallback: current id + TMDB episode number, so links
          //     never disappear even when the chain hasn't resolved yet.
          const resolveLink = (sn, n) => {
            const slot = seasonToAni.get(sn);
            if (slot) {
              let aniEp = null;
              const eps = slot.episodes || {};
              for (const k of Object.keys(eps)) {
                const e = eps[k];
                if (e && e.seasonNumber === sn && e.episodeNumber === n) {
                  aniEp = parseInt(k, 10);
                  break;
                }
              }
              if (aniEp == null) aniEp = n;
              return { aniId: slot.aniId, aniEp };
            }
            return { aniId: id, aniEp: n };
          };

          const renderPills = () => {
            pillsEl.innerHTML = seasons.map((s, i) => {
              const label = s.name && !/^season\s+\d+$/i.test(s.name) ? s.name : `Season ${s.season_number}`;
              return `<button class="an-season-pill ${i === activeIdx ? 'is-active' : ''}" data-i="${i}" title="${html(label)}">
                <span class="an-season-pill__n">${s.season_number}</span>
                <span class="an-season-pill__name">${html(label)}</span>
                <span class="an-season-pill__count">${s.episode_count} ep</span>
              </button>`;
            }).join('');
          };

          const renderSeasonEps = async (season, page) => {
            const sn = season.season_number;
            const count = season.episode_count || 0;
            if (!count) {
              grid.innerHTML = '<p class="an-empty" style="padding:24px;color:var(--text-mute)">No episodes listed.</p>';
              return;
            }

            const skelCount = Math.min(count, EPISODES_PER_PAGE, 8);
            grid.innerHTML = Array.from({ length: skelCount }, () =>
              `<div class="sk-episode"><div class="sk sk--still"></div><div class="sk sk--line sk--line-lg"></div><div class="sk sk--line sk--line-sm"></div></div>`
            ).join('');

            if (!seasonEpsCache[sn]) {
              const s = await tmdb(`/tv/${tmdbId}/season/${sn}`).catch(() => null);
              seasonEpsCache[sn] = (s && s.episodes) || [];
            }
            const tmdbEps = seasonEpsCache[sn];
            if (!grid.isConnected) return;

            // Make sure the relation chain has had a chance to register
            // AniList ids for OTHER seasons before we paint episode links.
            // (The current season already has its mapping from the start.)
            if (!seasonToAni.has(sn) && sn !== (curEpMap && curEpMap.seasonNumber)) {
              try { await chainPromise; } catch (_) {}
              if (!grid.isConnected) return;
            }

            // Default to whichever page the playing episode is on the first
            // time this season is opened; remember the user's chosen page
            // across season switches after that (same pattern as TV).
            const _defaultPage = () => {
              if (curEpMap && curEpMap.seasonNumber === sn) {
                const idx = tmdbEps.findIndex(e => e.episode_number === curEpMap.episodeNumber);
                if (idx >= 0) return Math.ceil((idx + 1) / EPISODES_PER_PAGE);
              }
              return 1;
            };
            const wantPage = page != null ? page : (pageBySeasonAnime[sn] || _defaultPage());
            const { page: p, pages, start, end } = paginate(tmdbEps.length, wantPage, EPISODES_PER_PAGE);
            pageBySeasonAnime[sn] = p;
            const slice = tmdbEps.slice(start, end);

            const cards = slice.map(e => {
              const n = e.episode_number;
              const { aniId, aniEp } = resolveLink(sn, n);
              const isCurrent = String(aniId) === String(id) && Number(aniEp) === Number(ep);
              const thumb = e.still_path
                ? `<img src="https://image.tmdb.org/t/p/w300${e.still_path}" loading="lazy">`
                : `<div class="episode__still-fallback"><i class="ri-film-line"></i></div>`;
              const meta = [
                e.air_date ? `<span>${e.air_date.slice(0, 7)}</span>` : '',
                e.runtime  ? `<span class="dot"></span><span>${e.runtime} min</span>` : '',
                e.vote_average > 0 ? `<span class="dot"></span><span><i class="ri-star-fill" style="color:var(--accent)"></i> ${e.vote_average.toFixed(1)}</span>` : ''
              ].filter(Boolean).join('');
              return `
                <a class="episode${isCurrent ? ' episode--playing' : ''}" href="/watch/anime/${aniId}/${aniEp}" data-link>
                  <div class="episode__still">
                    ${thumb}
                    <span class="episode__num">E${n}</span>
                    <span class="episode__playhint"><i class="ri-play-fill"></i></span>
                  ...
                  <div class="episode__body">
                    <div class="episode__title">${html(e.name || 'Episode ' + n)}</div>
                    ${meta ? `<div class="episode__meta">${meta}</div>` : ''}
                    ${e.overview ? `<div class="episode__overview">${html(e.overview)}</div>` : ''}
                  </div>
                </a>`;
            }).join('');

            grid.innerHTML = cards + pagerHtml(p, pages, start, end, tmdbEps.length, 'seasongo');
            $$('.ep-pager__btn[data-seasongo]', grid).forEach(btn => {
              btn.addEventListener('click', () => {
                const go = parseInt(btn.dataset.seasongo);
                if (Number.isFinite(go)) {
                  renderSeasonEps(season, go);
                  grid.scrollIntoView({ behavior: 'smooth', block: 'start' });
                }
              });
            });

            if (subEl) subEl.textContent = `${tmdbEps.length} episode${tmdbEps.length !== 1 ? 's' : ''}${season.name && !/^season\s+\d+$/i.test(season.name) ? ' · ' + season.name : ''}`;

            // Auto-scroll to currently playing episode card when it's in
            // this season — keeps the "what's playing" hint visible after
            // a season switch.
            requestAnimationFrame(() => {
              const playing = grid.querySelector('.episode--playing');
              if (playing) try { playing.scrollIntoView({ behavior: 'smooth', block: 'nearest' }); } catch (_) {}
            });
          };

          renderPills();
          renderSeasonEps(seasons[activeIdx]);
          ...
          // Once the cross-season chain resolves, repaint the active season
          // so its episode links pick up the correct AniList ids (the first
          // paint may have used fallback links).
          chainPromise.then(() => {
            if (!grid.isConnected) return;
            renderSeasonEps(seasons[activeIdx]);
          });
    ...
    if (!sameEp) {
      ...
      // Anime sources: movie-style providers (ScreenScape / Aurora / Nebula /
      // Lumen / Stellar / …) are the primary chain — they play via the
      // AniList → TMDB / IMDB mapping fetched from api.ani.zip inside
      // StellarPlayer.playAnime(). MegaPlay stays available but is folded
      // in the picker UI. Scraper workers (AnimeSalt / ToonStream / AnimeDekho)
      // append their embeds as they resolve and also land in the folded group.
      const movieChain = (typeof StellarPlayer.defaultChain === 'function'
        ? StellarPlayer.defaultChain()
        : []
      ).filter(s => s.kind !== 'megaplay' && s.kind !== 'embed');
      const animeServers = [
        ...movieChain,
      ...
      mini.activePlayer.playAnime(id, ep, { preferredName: getServerPref('anime') });
      mini.activeCtx = {
  ...
  function openSourceModal() {
    ...
    // rows are presented:
    //   • Anime: movie-style servers (ScreenScape / Aurora / Nebula / Lumen /
    //     Stellar / …) are the PRIMARY list — they play anime via the
    //     AniList → TMDB / IMDB mapping from api.ani.zip. MegaPlay and the
    //     scraped worker embeds (AnimeSalt / ToonStream / AnimeDekho) fold
    //     away under a collapsed "Load more servers" section.
    ...
    const isAnimeOnlySrc = (s) => s.kind === 'megaplay' || s.kind === 'embed' || /megaplay/i.test(s.name);

    // Anime cluster toggle: 'mega' shows MegaPlay + scraper-worker embeds,
    // 'demon' shows cinematic movie-style servers (ScreenScape / Aurora / …).
    const animeMode = isAnime ? getAnimeMode() : null;
    let primary, folded = [];
    if (isAnime) {
      const megaList  = allSources.filter(s =>  isAnimeOnlySrc(s));
      const demonList = allSources.filter(s => !isAnimeOnlySrc(s));
      primary = animeMode === 'mega' ? megaList : demonList;
    } else {
      primary = allSources;
    }

    const tagOf = (s) => (s.name.match(/\((Sub|Dub|Hindi)\)\s*$/i) || [, ''])[1].toLowerCase();
    const labelTag = (s) => {
      const t = tagOf(s);
      if (t === 'sub')   return 'SUB';
      if (t === 'dub')   return 'DUB';
      if (t === 'hindi') return 'HIN';
      if (s.kind === 'megaplay') return 'EMB';
      if (s.kind === 'embed')    return 'EMB';
      return 'HD';
    };
    const stripTag = (n) => n.replace(/\s*\((Sub|Dub|Hindi)\)\s*$/i, '');


    const rowHtml = (s) => {
      const active = s.name === curName;
      return `
      <button class="source-row src-row src-chip ${active ? 'active' : ''}" data-i="${s.index}" title="${html(s.name)}">
        <span class="src-chip__dot" aria-hidden="true">
          <i class="ri-${active ? 'volume-up-fill' : 'play-fill'}"></i>
        </span>
        <span class="src-chip__name">${html(stripTag(s.name))}</span>
        <span class="src-chip__tag">${labelTag(s)}</span>
      </button>`;
    };

    // Find which group contains the currently-playing source so we open
    // only that one by default — the rest stay folded for a compact view.
    const groupHasActive = (rows) => rows.some(r => r.name === curName);
    const collapsibleGroup = (g, openByDefault) => {
      const open = openByDefault || groupHasActive(g.rows);
      return `
        <section class="src-group src-group--col ${open ? 'is-open' : ''}">
          <button class="src-group__head" type="button" aria-expanded="${open}">
            <span class="src-group__title">${g.label}</span>
            <i class="src-group__rule"></i>
            <span class="src-pill">${g.rows.length}</span>
            <i class="ri-arrow-down-s-line src-group__chev"></i>
          </button>
          <div class="src-chip-grid src-group__body" ${open ? '' : 'hidden'}>${g.rows.map(rowHtml).join('')}</div>
        </section>`;
    };

    let primaryGroupsHtml;
    if (isAnime) {
      const order = [['sub', 'Japanese Subtitled'], ['dub', 'English Dubbed'], ['hindi', 'Hindi Dubbed']];
      const groups = [];
      const used = new Set();
      for (const [key, label] of order) {
        const rows = primary.filter(s => tagOf(s) === key);
        rows.forEach(r => used.add(r));
        if (rows.length) groups.push({ label, rows });
      }
      const rest = primary.filter(s => !used.has(s));
      if (rest.length) groups.push({ label: animeMode === 'mega' ? 'Mega cluster' : 'Other servers', rows: rest });
      const anyActive = groups.some(g => groupHasActive(g.rows));
      primaryGroupsHtml = groups.map((g, i) => collapsibleGroup(g, !anyActive && i === 0)).join('');
    } else {
      primaryGroupsHtml = primary.length
        ? collapsibleGroup({ label: 'Available servers', rows: primary }, true)
        : '';
    }
    const foldedRows  = folded.map(rowHtml).join('');

    // Anime-only: manual "pick the matching title" rows for each scraper
    // worker. Only relevant while the Mega cluster is selected.
    const animeWorkers = (isAnime && animeMode === 'mega' && Array.isArray(_CFG.ANIME_WORKERS)) ? _CFG.ANIME_WORKERS : [];
    let matchRows = '';
    if (animeWorkers.length) {
      ...
      matchRows = animeWorkers.map((w) => {
        ...
        const tag = _langTag(w.lang);
        return `
          <button class="source-row src-row src-row--match ${isMatched ? 'is-matched' : ''}" data-anime-match="${html(w.name)}">
            <div class="source-row__tile">
              <i class="ri-${isMatched ? 'refresh-line' : 'search-line'}"></i>
            </div>
            <div class="source-row__body">
              <div class="source-row__name">
                <span class="source-row__title">${html(w.name)}</span>
              </div>
              <div class="source-row__sub">${isMatched ? 'Matched · tap to re-pick title' : 'Tap to choose the matching title'}</div>
            </div>
            <div class="source-row__tag">${tag.toUpperCase()}</div>
          </button>`;
      }).join('');
    }

    const total = primary.length;
    const megaCount  = isAnime ? allSources.filter(s =>  isAnimeOnlySrc(s)).length : 0;
    const demonCount = isAnime ? allSources.filter(s => !isAnimeOnlySrc(s)).length : 0;

    // Telegram / ads / monetization chips (config-driven).
    const _LK = (_CFG.LINKS || {});
    const linkChip = (k, icon) => {
      const v = _LK[k];
      if (!v || !v.url) return '';
      return `<a class="src-link-chip src-link-chip--${k.toLowerCase()}" href="${html(v.url)}" target="_blank" rel="noopener"><i class="ri-${icon}"></i><span>${html(v.label || k)}</span></a>`;
    };
    const linksHtml = [linkChip('TELEGRAM', 'telegram-fill'), linkChip('ADS', 'megaphone-line'), linkChip('MONETIZE', 'heart-3-line')].filter(Boolean).join('');

    const animeToggleHtml = isAnime ? `
      <div class="src-mode" role="tablist" aria-label="Anime server cluster">
        <button class="src-mode__btn ${animeMode === 'demon' ? 'is-active' : ''}" data-mode="demon" role="tab" aria-selected="${animeMode === 'demon'}">
          <i class="ri-sparkling-2-fill"></i><span>Demon</span><em>${demonCount}</em>
        </button>
        <button class="src-mode__btn ${animeMode === 'mega' ? 'is-active' : ''}" data-mode="mega" role="tab" aria-selected="${animeMode === 'mega'}">
          <i class="ri-flashlight-fill"></i><span>Mega</span><em>${megaCount}</em>
        </button>
      </div>` : '';

    m.innerHTML = `
      <div class="source-modal__panel">
        <div class="source-modal__grip" aria-hidden="true"></div>
        <div class="source-modal__head">
          <div class="source-modal__title">
          ...
          <button class="src-close" data-close aria-label="Close"><i class="ri-close-line"></i></button>
        </div>
        ...
        <div class="source-modal__scroll">
          ...
          ${matchRows ? `
            <section class="src-group">
              <div class="src-group__label"><span><i class="ri-links-line"></i> Add source</span><i class="src-group__rule"></i><span class="src-pill">${animeWorkers.length}</span></div>
              <div class="source-rows">${matchRows}</div>
            </section>` : ''}

          <div class="source-modal__hint">
            <i class="ri-shield-keyhole-line"></i>
            <span>Having trouble? Switch servers or try a different language track.</span>
          </div>

          ${linksHtml ? `<div class="src-links">${linksHtml}</div>` : ''}
        </div>
      </div>`;
    document.body.appendChild(m);
    ...
    // Anime cluster toggle (Demon / Mega). Persists pick, re-renders modal,
    // and if the currently-playing source isn't in the new cluster, switches
    // to that cluster's first server so playback follows the toggle.
    $$('#sourceModal .src-mode__btn').forEach((btn) => {
      btn.addEventListener('click', () => {
        const mode = btn.dataset.mode === 'mega' ? 'mega' : 'demon';
        if (mode === getAnimeMode()) return;
        setAnimeMode(mode);
        const sources = mini.activePlayer.listSources();
        const wantMega = mode === 'mega';
        const inCluster = sources.filter(s => wantMega === isAnimeOnlySrc(s));
        const cur = mini.activePlayer.currentSourceName();
        if (!inCluster.some(s => s.name === cur) && inCluster.length) {
          mini.activePlayer.setSource(inCluster[0].index);
          if (mini.activePlayer.setPreferredName) mini.activePlayer.setPreferredName(inCluster[0].name);
          setServerPref('anime', inCluster[0].name);
          if ($('#sourceName')) $('#sourceName').textContent = inCluster[0].name;
        }
        m.remove();
        openSourceModal();
      });
    ...
    // Collapsible primary groups (one click on the section header toggles).
    $$('#sourceModal .src-group--col .src-group__head').forEach((head) => {
      head.addEventListener('click', () => {
        const sec = head.closest('.src-group--col');
        if (!sec) return;
        const body = sec.querySelector('.src-group__body');
    ...
    $$('#sourceModal .source-row[data-i]').forEach(row => {
      row.addEventListener('click', () => {
        ...
        const picked = mini.activePlayer.currentSourceName() || '—';
        if ($('#sourceName')) $('#sourceName').textContent = picked;
        // Persist user's choice — restored next time they open any content
        // of the same type (movie / tv / anime).
        if (mini.activeCtx && mini.activeCtx.type) {
          setServerPref(mini.activeCtx.type, picked);
          if (mini.activePlayer.setPreferredName) mini.activePlayer.setPreferredName(picked);
        }
        closeModal();
  ...
  // Returns an array of hits, or `null` specifically when AniList couldn't be
  // reached at all (distinct from a real "zero results" empty array) so the
  // caller can show "AniList is unreachable" instead of "no anime found".
    ...
    if (scope === 'anime') {
      ...
      if (media === null) {
        $('#searchResults').innerHTML = `<div class="empty">
          <div class="empty__icon"><i class="ri-wifi-off-line"></i></div>
          <h3 class="serif"><em>Couldn't reach AniList.</em></h3>
          <p>It may be rate-limiting requests right now.</p>
          <button class="btn-ghost" id="animeSearchRetry"><i class="ri-refresh-line"></i> Retry</button>
        </div>`;
        $('#animeSearchRetry')?.addEventListener('click', () => onSearch(), { once: true });
        return;
      }
      if (!media.length) {
  ...
  // Splash used to hide on a flat 1100ms timer regardless of whether the
  // page was actually ready, which is backwards on a first/cold load (this
  // file alone is ~190KB before the network or CDN fonts are even warm):
  // the splash would vanish onto an unfinished or still-skeleton screen.
  // Now it stays up until the initial route has actually finished
  // rendering, with only a small minimum hold so it never just flashes.
  const SPLASH_MIN_MS = 500;
  const splashStart = Date.now();
  const splashStatusTimer = setTimeout(() => {
    const el = $('#splashStatus');
    if (el) { el.textContent = 'Still loading — first visit can take a moment'; el.classList.add('is-visible'); }
  }, 4000);

  function hideSplash() {
    clearTimeout(splashStatusTimer);
  ...
  window.OMNIFLIX = {
    ...
    removeContinue: (id) => {
      removeProgress(id);
      const pn = parsePath().name;
      if (pn === 'home') { route(); }
      else {
        // Refresh browse-page continue section in-place if it's showing
        const secBrowse = $('#sec-browse-continue');
        if (secBrowse) {
          const browseType = browseState && browseState.type;
          const progStore = getProgressStore();
          const items = Object.values(progStore)
            .filter(x => x && x.type === browseType && x.progress && x.progress.duration > 0 && (x.progress.watched / x.progress.duration) < 0.97)
            .sort((a, b) => (b.last_updated || 0) - (a.last_updated || 0))
            .slice(0, 8);
          if (items.length) {
            secBrowse.querySelector('.rail__strip').innerHTML = items.map(continueCard).join('');
          } else {
            secBrowse.remove();
          }
        }
